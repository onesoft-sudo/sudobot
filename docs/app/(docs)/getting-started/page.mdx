...
Or if using bun, you can run the following command to build the bot:

```bash
bun run build
```

If that fails, don't worry. You can get a pre-built version or run the bot directly without building, you just might encounter some errors with a few commands. If this does happen, you can get help in the [Discord Server](https://discord.gg/892GWhTzgs).

If it fails at building because you don't have enough resources to do so, or if it's for another reason, you can find pre-built versions [here](github.com/onesoft-sudo/sudobot/releases/latest) for linux and darwin.
...
Or if you are running bun, you can skip this step and move onto the enviroment variables.
...
Or in a production enviroment, it's best to use the following command as it uses a process manager.
...
Or if you are using bun, you can start the bot up using the following command:
...
```bash
bun dev:prod --no-daemon
```

If the bot starts up and comes online, and you don't want the real time logging, you can remove `--no-daemon` so your command looks like this:
