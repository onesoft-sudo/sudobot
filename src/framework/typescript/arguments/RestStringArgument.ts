...
class RestStringArgument extends Argument<string> {
    ...
    public override resolveFromRawValue() {
        assert(this.context instanceof LegacyContext);

        let content = this.context.commandContent.trim();
        let i = 0;

        for (const arg of this.context.isLegacy() ? this.context.argv : []) {
    ...
    public override postValidate(): boolean {
        ...
        if (typeof rangeMinRuleValue === "number" && this.value!.length < rangeMinRuleValue) {
            return this.error(
                this.getRuleErrorMessage("range:min") ?? `Argument '${this.name}': String value is too short`,
                ArgumentErrorType.InvalidRange
            );
        }

        if (typeof rangeMaxRuleValue === "number" && this.value!.length > rangeMaxRuleValue) {
            return this.error(
                this.getRuleErrorMessage("range:max") ?? `Argument '${this.name}': String value is too long`,
                ArgumentErrorType.InvalidRange
            );
        }
