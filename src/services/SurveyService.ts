...
export const name = "surveyService";

export default class SurveyService extends Service implements HasEventListeners {
    @GatewayEventListener("interactionCreate")
    public async onInteractionCreate(interaction: Interaction<CacheType>) {
        const config = this.client.configManager.config[interaction.guildId!]?.survey;
    ...
    public async onModalSubmit(interaction: ModalSubmitInteraction<CacheType>) {
        ...
        if (!logChannelId) {
            this.client.logger.warn("No log channel specified for survey completion.");
            return;
        }
    ...
    private checkPreconditions(
        ...
        surveyConfig: NonNullable<GuildConfig["survey"]>["surveys"][string]
    ) {
        return (
            surveyConfig &&
