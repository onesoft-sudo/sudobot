...
class InviteTrackingService extends Service implements HasEventListeners {
    ...
    private readonly _uses = new Collection<
        `${Snowflake}::${string}`,
        number
    >();

    public get invites(): ReadonlyCollection<
        `${Snowflake}::${string}`,
        InviteInfo
    > {
    ...
    public onGuildUpdate(oldGuild: Guild, newGuild: Guild) {
        if (oldGuild.vanityURLCode !== newGuild.vanityURLCode) {
            if (oldGuild.vanityURLCode) {
                this._invites.delete(
                    `${oldGuild.id}::${oldGuild.vanityURLCode}`
                );
            }
    ...
    public async findInviteForMember(member: GuildMember) {
        ...
        for (const invite of invites.values()) {
            const existingUses = this._uses.get(
                `${member.guild.id}::${invite.code}`
            );
