...
class InfractionManagerService extends Service {
    ...
    private readonly actionDoneNames: Record<InfractionType, string> = {
        ...
        [InfractionType.ModeratorMessage]: "sent a moderator message",
    ...
    private readonly typeToPointKeyMap = {
        ...
        [InfractionType.ModeratorMessage]: "moderator_message",
        ...
        [K in InfractionType]?: keyof NonNullable<
            GuildConfigurationType["infractions"]
        >["points"];
    };
    ...
    public async processReason(
        ...
        const templates = config?.infractions?.reason_templates ?? {};
        const templateWrapper =
            config?.infractions?.reason_template_placeholder_wrapper ??
            "{{%name%}}";
    ...
    private async notify(
        ...
        const transformed = transformNotificationEmbed
            ...
            : embed;
        const attachmentStoragePath = systemPrefix("storage/attachments");
        ...
        for (const attachment of attachments) {
            ...
            if (!_troll) {
                await downloadFile({
                    ...
                    path: systemPrefix("storage/attachments")
                });
            }
                        ...
                        let [newInfraction] = await tx
                            ...
                            .values({
                                ...
                                reason: processReason
                                    ? await this.processReason(guildId, reason)
                                    : reason,
                                type,
                                deliveryStatus:
    ...
    public async updateReasonById(
        ...
        reason = (await this.processReason(guildId, reason)) ?? reason;
    ...
    public getLocalAttachmentPath(attachment: string) {
        const storagePath = systemPrefix("storage/attachments");
    ...
    public prettifyInfractionType(type: InfractionType) {
        switch (type) {
            ...
            case InfractionType.ModeratorMessage:
    ...
    public async createShot<E extends boolean>(
        ...
        const infraction: Infraction = {
            ...
            moderatorId: moderator.id,
            userId: user.id,
            reason: await this.processReason(guildId, reason),
    ...
    public async createUnban<E extends boolean>(
        ...
        const reason =
            (await this.processReason(guildId, rawReason)) ?? rawReason;
        ...
        try {
            await this.queueService.bulkCancel(UnbanQueue, job => {
                return (
                    job.data?.userId === user.id &&
                    job.data?.guildId === guild.id &&
    ...
    public async createMute<E extends boolean>(
        ...
        if (
            (mode === "role" && role && member.roles.cache.has(role)) ||
        ...
        if (mode === "role" && role) {
            await this.queueService.bulkCancel(UnmuteQueue, queue => {
                return (
                    ...
                    queue.data?.guildId === guild.id &&
                    queue.state !== JobState.Executing
                );
            });
        ...
        return {
            ...
            summarize: () => {
                ...
                for (const type of keys) {
                    ...
                    summary += `**${infractions.length}** ${type === this.typeToPointKeyMap[InfractionType.ModeratorMessage] ? "moderator message" : type}${index === keys.length - 1 ? "" : ", "}`;
                    index++;
...
export type InfractionStatistics = {
    total: number;
    points: Record<
        keyof NonNullable<GuildConfigurationType["infractions"]>["points"],
        number
    >;
    infractions: Record<
        keyof NonNullable<GuildConfigurationType["infractions"]>["points"],
...
type InfractionConfig = NonNullable<GuildConfigurationType["infractions"]>;
...
export default InfractionManagerService;
