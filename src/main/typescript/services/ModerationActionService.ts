...
class ModerationActionService extends Service {
        ...
        for (const action of actions) {
            ...
            const result =
                isMemberOnlyAction && target instanceof GuildMember
                    ? await this.takeActionOnMember(
                          guild,
                          target,
                          action as MemberOnlyAction
                      )
    ...
    private async takeActionOnMember(
        guild: Guild,
        target: GuildMember,
        ...
        switch (action.type) {
            case "kick":
                return await this.infractionManagerService.createKick({
            ...
            case "mute":
                return await this.infractionManagerService.createMute({
                    moderator: this.application.client.user!,
                    member: target,
                    reason: action.reason,
            ...
            case "role":
                return await this.infractionManagerService.createRoleModification(
                    {
                        moderator: this.application.client.user!,
                        member: target,
                        reason: action.reason,
            ...
            case "warn":
                return await this.infractionManagerService.createWarning({
                    moderator: this.application.client.user!,
                    member: target,
                    reason: action.reason,
                    guildId: guild.id,
    ...
    private async takeActionOnUser(
        ...
        switch (action.type) {
            case "ban":
                return await this.infractionManagerService.createBan({
                    moderator: this.application.client.user!,
                    user: target,
                    reason: action.reason,
            ...
            case "clear":
                ...
                return await this.infractionManagerService.createClearMessages({
                    moderator: this.application.client.user!,
                    user: target,
                    reason: action.reason,
                    guildId: guild.id,
            ...
            case "delete_message":
                if (!message) {
                    throw new Error(
                        "Message is required for delete_message action"
                    );
                }
