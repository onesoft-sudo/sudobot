import {
    ...
    ApplicationCommandType, Awaitable, Channel,
    ChatInputCommandInteraction,
    ContextMenuCommandBuilder,
    ContextMenuCommandInteraction,
    InteractionDeferReplyOptions, InteractionEditReplyOptions,
    InteractionResponse,
    Message, MessageCreateOptions, MessagePayload, ModalSubmitInteraction,
    PermissionResolvable,
    SlashCommandBuilder, TextBasedChannel, User
} from "discord.js";
import Client from "./Client";
import { AnyCommandContext, CommandMessage, CommandReturn } from "./Command"; // FIXME
...
type DeferReplyOptions = ((MessageCreateOptions | MessagePayload | InteractionEditReplyOptions) & { ephemeral?: boolean }) | string;
type PermissionValidationResult = boolean | {
    isPermitted: boolean;
    source?: string;
};
...
export default abstract class CommandV2 {
    ...
    protected static commandArgumentParser = new CommandArgumentParser(Client.instance);
    ...
    protected async deferredReply(message: CommandMessage, options: DeferReplyOptions, mode: DeferReplyMode = "default") {
    ...
    protected validatePermissions(): Awaitable<PermissionValidationResult> {
        // TODO
        return true;
    }

    protected async doChecks({ message }: RunCommandOptions) {
        const permissionValidationResult = await this.validatePermissions();
        const isPermitted =
        ...
        const debugMode = (this.client.configManager.systemConfig.debug_mode || this.client.configManager.config[message.guildId!]?.debug_mode) ?? false;

        if (!isPermitted) {
            await this.error(message, `You don't have permission to run this command.${debugMode && permissionValidationFailureSource ? `\nSource: \`${permissionValidationFailureSource}\`` : ""}`);
            return false;
        }
    ...
    protected async parseArguments({ message, context } : RunCommandOptions) {
        ...
        const { error, parsedArgs } = await CommandV2.commandArgumentParser.parse({
            message,
